export const BUG_FIX_WORKER_RESULT_JSON_BLOCK_TAG = "bug-fix-worker-result-json";
export const BUG_FIX_WORKER_RESULT_CONTRACT_VERSION = 1;

export type BugFixWorkerResultStatus = "completed" | "blocked" | "failed";

export interface BugFixWorkerValidation {
  command: string;
  outcome: "passed" | "failed" | "not_run";
  details?: string;
}

interface BugFixWorkerResultBase {
  version: 1;
  kind: "bug_fix_worker_result";
  unitId: string;
  status: BugFixWorkerResultStatus;
  summary: string;
  validations: BugFixWorkerValidation[];
}

export interface CompletedBugFixWorkerResult extends BugFixWorkerResultBase {
  status: "completed";
  changedFiles: string[];
}

export interface BlockedBugFixWorkerResult extends BugFixWorkerResultBase {
  status: "blocked";
  blockers: string[];
}

export interface FailedBugFixWorkerResult extends BugFixWorkerResultBase {
  status: "failed";
  blockers: string[];
}

export type BugFixWorkerResult =
  | CompletedBugFixWorkerResult
  | BlockedBugFixWorkerResult
  | FailedBugFixWorkerResult;

export type BugFixWorkerResultParseErrorCode =
  | "missing_block"
  | "malformed_json"
  | "invalid_schema";

export interface BugFixWorkerResultParseSuccess {
  ok: true;
  value: BugFixWorkerResult;
  rawJson: string;
}

export interface BugFixWorkerResultParseError {
  ok: false;
  code: BugFixWorkerResultParseErrorCode;
  message: string;
}

export type BugFixWorkerResultParseResult =
  | BugFixWorkerResultParseSuccess
  | BugFixWorkerResultParseError;

type ValidationResult<T> = { ok: true; value: T } | BugFixWorkerResultParseError;

export function extractTaggedJsonBlock(
  text: string,
  tag: string = BUG_FIX_WORKER_RESULT_JSON_BLOCK_TAG,
): string | undefined {
...
export function parseTaggedWorkerResult(text: string): BugFixWorkerResultParseResult {
  const rawJson = extractTaggedJsonBlock(text);
  if (!rawJson) {
    return {
      ok: false,
      code: "missing_block",
      message: `Missing tagged JSON block \`${BUG_FIX_WORKER_RESULT_JSON_BLOCK_TAG}\`.`,
    };
  }
...
export function validateWorkerResult(value: unknown): ValidationResult<BugFixWorkerResult> {
  if (!isRecord(value)) {
    return invalidSchema("Tagged JSON block must be an object.");
  }

  if (value.version !== BUG_FIX_WORKER_RESULT_CONTRACT_VERSION) {
    return invalidSchema(
      `Worker result version must be ${BUG_FIX_WORKER_RESULT_CONTRACT_VERSION}.`,
    );
  }

  if (value.kind !== "bug_fix_worker_result") {
    return invalidSchema("Worker result kind must be 'bug_fix_worker_result'.");
  }
  ...
  if (value.status === "completed") {
    ...
    return {
      ok: true,
      value: {
        version: BUG_FIX_WORKER_RESULT_CONTRACT_VERSION,
        kind: "bug_fix_worker_result",
        unitId: unitId.value,
        status: "completed",
...
function readValidations(value: unknown): ValidationResult<BugFixWorkerValidation[]> {
  if (!Array.isArray(value)) {
    return invalidSchema("Worker result validations must be an array.");
  }

  const validations: BugFixWorkerValidation[] = [];
...
function invalidSchema(message: string): BugFixWorkerResultParseError {
  return {
    ok: false,
    code: "invalid_schema",
    message,
