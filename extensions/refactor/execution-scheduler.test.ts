...
import { buildExecutionLayers, RefactorExecutionScheduler } from "./execution-scheduler";

function createUnits(): RefactorExecutionUnit[] {
  return [
    {
      id: "unit-a",
      title: "Unit A",
      objective: "Run independently in layer 1.",
      targets: ["a.ts"],
      validations: ["test a"],
      dependsOn: [],
    },
...
test("scheduler dispatches independent units in the same layer concurrently", async () => {
  ...
  let active = 0;
  let maxActive = 0;
  const scheduler = new RefactorExecutionScheduler({
    executor: {
      async executeUnit({ executionUnit }) {
        ...
        return {
          ...
          changedFiles: [executionUnit.targets[0]!],
          validations: [],
        };
      },
    },
