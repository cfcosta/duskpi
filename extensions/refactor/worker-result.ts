export const REFACTOR_WORKER_RESULT_JSON_BLOCK_TAG = "refactor-worker-result-json";
export const REFACTOR_WORKER_RESULT_CONTRACT_VERSION = 1;

export type RefactorWorkerResultStatus = "completed" | "blocked" | "failed";

export interface RefactorWorkerValidation {
  command: string;
  outcome: "passed" | "failed" | "not_run";
  details?: string;
}

interface RefactorWorkerResultBase {
  version: 1;
  kind: "refactor_worker_result";
  unitId: string;
  status: RefactorWorkerResultStatus;
  summary: string;
  validations: RefactorWorkerValidation[];
}

export interface CompletedRefactorWorkerResult extends RefactorWorkerResultBase {
  status: "completed";
  changedFiles: string[];
}

export interface BlockedRefactorWorkerResult extends RefactorWorkerResultBase {
  status: "blocked";
  blockers: string[];
}

export interface FailedRefactorWorkerResult extends RefactorWorkerResultBase {
  status: "failed";
  blockers: string[];
}

export type RefactorWorkerResult =
  | CompletedRefactorWorkerResult
  | BlockedRefactorWorkerResult
  | FailedRefactorWorkerResult;

export type RefactorWorkerResultParseErrorCode =
  | "missing_block"
  | "malformed_json"
  | "invalid_schema";

export interface RefactorWorkerResultParseSuccess {
  ok: true;
  value: RefactorWorkerResult;
  rawJson: string;
}

export interface RefactorWorkerResultParseError {
  ok: false;
  code: RefactorWorkerResultParseErrorCode;
  message: string;
}

export type RefactorWorkerResultParseResult =
  | RefactorWorkerResultParseSuccess
  | RefactorWorkerResultParseError;

type ValidationResult<T> = { ok: true; value: T } | RefactorWorkerResultParseError;

export function extractTaggedJsonBlock(
  text: string,
  tag: string = REFACTOR_WORKER_RESULT_JSON_BLOCK_TAG,
): string | undefined {
...
export function parseTaggedWorkerResult(text: string): RefactorWorkerResultParseResult {
  const rawJson = extractTaggedJsonBlock(text);
  if (!rawJson) {
    return {
      ok: false,
      code: "missing_block",
      message: `Missing tagged JSON block \`${REFACTOR_WORKER_RESULT_JSON_BLOCK_TAG}\`.`,
    };
  }
...
export function validateWorkerResult(value: unknown): ValidationResult<RefactorWorkerResult> {
  if (!isRecord(value)) {
    return invalidSchema("Tagged JSON block must be an object.");
  }

  if (value.version !== REFACTOR_WORKER_RESULT_CONTRACT_VERSION) {
    return invalidSchema(
      `Worker result version must be ${REFACTOR_WORKER_RESULT_CONTRACT_VERSION}.`,
    );
  }

  if (value.kind !== "refactor_worker_result") {
    return invalidSchema("Worker result kind must be 'refactor_worker_result'.");
  }
  ...
  if (value.status === "completed") {
    ...
    return {
      ok: true,
      value: {
        version: REFACTOR_WORKER_RESULT_CONTRACT_VERSION,
        kind: "refactor_worker_result",
        unitId: unitId.value,
        status: "completed",
...
function readValidations(value: unknown): ValidationResult<RefactorWorkerValidation[]> {
  if (!Array.isArray(value)) {
    return invalidSchema("Worker result validations must be an array.");
  }

  const validations: RefactorWorkerValidation[] = [];
...
function invalidSchema(message: string): RefactorWorkerResultParseError {
  return {
    ok: false,
    code: "invalid_schema",
    message,
