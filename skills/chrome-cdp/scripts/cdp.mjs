...
function masterSockPath(port) {
  return `/tmp/cdp-master-${port}.sock`;
}
...
function getBrowserCandidates(browserName) {
  if (browserName) {
    ...
    if (!profile)
      throw new Error(
        `Unknown browser "${browserName}". Known: ${Object.keys(BROWSER_PROFILES).join(", ")}`,
      );
    ...
    if (fallbackPort && fallbackPort !== parseInt(filePort)) {
      ...
      if (fallbackUrl) {
        process.stderr.write(
          `Note: DevToolsActivePort was stale (port ${filePort}), found ${gBrowser} on port ${fallbackPort}\n`,
        );
        return fallbackUrl;
      }
  ...
  if (cmd === "use") {
    ...
    if (/^\d+$/.test(val)) {
      ...
      if (!BROWSER_PROFILES[key]) {
        console.error(
          `Unknown browser "${val}". Known: ${Object.keys(BROWSER_PROFILES).join(", ")}`,
        );
        process.exit(1);
      }
