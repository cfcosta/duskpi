...
export interface WorkerResultParseSuccess<Result> {
  ok: true;
  value: Result;
}

export interface WorkerResultParseError {
  ok: false;
  message: string;
}

export type WorkerResultParseResult<Result> =
  | WorkerResultParseSuccess<Result>
  | WorkerResultParseError;

export interface WorkerRunnerOptions<Result> {
  exec: WorkerRunnerExec;
  parseResult: (assistantText: string) => WorkerResultParseResult<Result>;
  command?: string;
  timeoutMs?: number;
}
...
export class WorkerRunner<Result> {
  private readonly command: string;
  private readonly timeoutMs: number;

  constructor(private readonly options: WorkerRunnerOptions<Result>) {
    this.command = options.command ?? "pi";
    this.timeoutMs = options.timeoutMs ?? DEFAULT_WORKER_TIMEOUT_MS;
  }

  async run(input: WorkerRunInput): Promise<Result> {
    ...
    const parsed = this.options.parseResult(assistantText);
    if (!parsed.ok) {
