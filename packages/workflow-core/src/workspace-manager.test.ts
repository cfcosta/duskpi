  ...
  assert.deepEqual(calls, [
    {
      ...
      options: { cwd: "/repo", timeout: 5000, signal: undefined },
    },
  ]);
});
...
test("updateStaleWorkspace looks up the workspace root and runs jj workspace update-stale there", async () => {
  ...
  assert.deepEqual(calls, [
    ...
    },
    {
      ...
      options: { cwd: "/tmp/workspaces/refactor-c", timeout: 15000, signal: undefined },
    },
  ]);
});
...
test("createWorkspace surfaces jj failures with stderr details", async () => {
  const { exec } = createExecMock([{ code: 1, stderr: "destination exists" }]);
  const manager = new JjWorkspaceManager({
    repoRoot: "/repo",
    exec,
  });
  ...
  await assert.rejects(
    () => manager.createWorkspace("refactor-e", "/tmp/workspaces/refactor-e"),
    /jj workspace add .*destination exists/i,
  );
});
